...
### Problem 16 
Let $\gcd(L)$ and $\operatorname{lcm}(L)$ be the greatest common divisor and least common multiple functions respectively. Which of the following statements are always true for positive integers variables $a,b,c$?
...
### Problem 17 
Consider the set of points on the coordinate plane $S: y=\sqrt{-x^{2}+4x}$. There exist real values of $a$ such that $S$ and $y=x+a$ have exactly one intersection; furthermore, the set of possible values of $a$ can be represented as $a\in [m,n) \cup \{\sqrt{a}-b\}$ for integers $m,n,a,b$. Compute $m+n+a+b$.
